odoo.define('muti_pos_mechanic.MechanicScreen', function(require) {
    'use strict';
    ...
    class ProductScreen extends ControlButtonsMixin(PosComponent) {
        ...
        async _getAddProductOptions(product) {
            ...
            if (['serial', 'lot'].includes(product.tracking) && (this.env.pos.picking_type.use_create_lots || this.env.pos.picking_type.use_existing_lots)) {
				...
				//alert(isAllowOnlyOneLot)
				//alert(packLotLinesToEdit)
        ...
        _onClickPay() {
			...
			this.showPopup('SalesRepPopup', {
		            title: this.env._t('Select Sales Representative'),
		            users: this.env.pos.users,
		            pos: this.env.pos,
		    });
